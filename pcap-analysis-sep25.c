        ...
        if (ntohs(eheader.ethertype) == 0x0800 || ntohs(eheader.ethertype) == 0x86DD) { 
            ...
            if (next_header_protocol == TCP_TYPE_NUM && ip_valid) {
                ...
                if ((tcpheader.flags & TCP_FLAG_SYN) && !session_initiated) {
                    ...
                    session_initiated = 1;
                    TCP_session_count = 0;
                    session_start_time = pkt_header.time_usec;
                } 
                ...
                if (session_initiated) {
                    ...
                    // Record ips
                    fprintf(fd_out, "%s\t%s\t", src_ip_str,dst_ip_str);

                    // Print the remaining TCP session details
                    session_through_bytes += (caplen * 8);
                    session_goodput_bytes += (payload_length * 8); 
                    fprintf(fd_out, "%d\t%d\t", ntohs(tcpheader.source_port),ntohs(tcpheader.destination_port)); 
                    fprintf(fd_out, "%d\t%d\t%d\t", num_packets_sent,session_through_bytes,session_goodput_bytes);    

                    // Calculate session duration and throughput
                    char sec_str[20];  // Create a buffer to store the formatted string
                    double sessionDuration = (pkt_header.time_usec - session_start_time)/1000000.0;
                    snprintf(sec_str, sizeof(sec_str), "%.4f", sessionDuration);
                    fprintf(fd_out, "%s\t", sec_str);  // Session duration
                    
                    double total_time = (pkt_header.time_usec - time_zero)/1000000.0;


                    fprintf(fd_out, "%f\t", session_through_bytes / sessionDuration);  // IP Layer Throughput
                    fprintf(fd_out, "%f\t", session_goodput_bytes / sessionDuration);  // Goodput
                    fprintf(fd_out, "%d\t\n", frame_number);
                    
                    // char wireshark_timestamp[20];
                    // snprintf(wireshark_timestamp, sizeof(wireshark_timestamp), "%.9f", total_time);
                    // fprintf(fd_out,"%s\t\n",wireshark_timestamp);
                    ...
                    if (tcpheader.flags & TCP_FLAG_FIN) {
                        ...
                        session_initiated = 0;
                        num_packets_sent = 0;
                        session_through_bytes = 0;
                        session_goodput_bytes = 0;
